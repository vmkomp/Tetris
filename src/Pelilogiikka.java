...
public class Pelilogiikka extends JPanel implements KeyListener, ActionListener {
	...
	PeliIkkuna peliIkkuna = new PeliIkkuna();
	...
	public void asetaKonstruktori(int[][] ladattuPeli) {
		rivi = 20;
		sarake = 10;
		pelilauta = new Pelilauta(rivi, sarake, ladattuPeli);
	...
	public void kaannaMuoto() {
		...
		if (iteroituTaulukko.saakoLiikkua() && iteroituTaulukko2.saakoLiikkua()) {
			paivitaTaulukkoRotaatio(iteroituTaulukko.pisteet(), iteroituTaulukko.normalisointiX(),
					iteroituTaulukko.normalisointiY());
		}
				...
				for (int j = sarake - 1; j >= 0; j--) {
					if (onkointattuLiikkuva(i, j, 1)) {
						...
						try {
							...
							// exceptionin tapahtuessa on ulkona pelilaudasta, jolloin halutaan saada liike
							// lakkaamaan.
	...
	public void paivitaTaulukko(boolean saakoLiikkua, ArrayList<Piste> pisteet, Liikkumistyyppi liikkumistyyppi,
			int lisaaX, int lisaaY) {
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if (!peliLoppuu()) {
			...
			System.out.println("Nyt pit‰isi avata TulosIkkuna ja lopettaa pelin logiikka");
			timer.stop();
			peliIkkuna.piilotaPeliIkkuna();
	...
	// Tarkastetaan onko ylimm‰ll‰ rivill‰ staattinen palikka
	// T‰llˆin pelin on loputtava

	public boolean peliLoppuu() {

		int ekanRivinYkkostenMaara = 0;

		staattinenTaulukko = pelilauta.annaStaattinenTaulukko();

		for (int i = 0; i < 10; i++) {
			// System.out.println(staattinenTaulukko[0][0]);
			ekanRivinYkkostenMaara += staattinenTaulukko[0][i];
		}

		if (ekanRivinYkkostenMaara != 0) {
			System.out.println("Peli loppu!");
			return true;
		} else {
			// System.out.println("Peli ei loppunut!");
			return false;
		}
